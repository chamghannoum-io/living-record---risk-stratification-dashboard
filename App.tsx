...
import TrendSurveillanceView from './components/Trends/TrendSurveillanceView';
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<AppTab>('population');
          ...
          {activeTab === 'dashboard' && (
            <DashboardView
              stats={{
                ...
                highRiskCount: filteredPatients.filter(p => p.riskLevel === RiskLevel.CRITICAL || p.riskLevel === RiskLevel.HIGH).length,
                highRiskPercent: filteredPatients.length ? ((filteredPatients.filter(p => p.riskLevel === RiskLevel.CRITICAL || p.riskLevel === RiskLevel.HIGH).length / filteredPatients.length) * 100).toFixed(1) : '0',
                avgScore: filteredPatients.length ? (filteredPatients.reduce((sum, p) => sum + p.riskScore, 0) / filteredPatients.length).toFixed(1) : '0',
                complexNeeds: filteredPatients.filter(p => p.comorbiditiesCount >= 3).length
              }}
              filters={filters}
